...
public class ProjectSecurityConfig {
	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) {
		http/*.requiresChannel(rcc -> rcc.anyRequest().requiresInsecure())  // forcing HTTP only ! */
				.sessionManagement(smc -> smc.invalidSessionUrl("/invalidSession").maximumSessions(3).maxSessionsPreventsLogin(true)/*.expiredUrl("/exporedUrl")*/) // will redirect to '/invalidSession' after timeout
